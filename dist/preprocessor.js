 ...
 *   // → '- Phase: Discovery\n\nNext'
 */
export function cleanAssistantMarkdown(raw, options = {}) {
...
function transformSegment(text, opts) {
    let out = text;
    // Order matters: normalize headings before stripping bold, so the
    // heading detector can see the original markup.
    if (opts.normalizeAllCapsHeadings)
        out = normalizeAllCapsHeadings(out);
    if (opts.flattenHeadingExplosion)
        out = flattenHeadingExplosion(out);
    if (opts.stripDecorativeRules)
        out = stripDecorativeRules(out);
    if (opts.normalizeListLabels)
        out = normalizeListLabels(out);
    if (opts.stripDefaultBold)
        out = stripDefaultBold(out);
    if (opts.normalizeBangs)
        out = normalizeBangs(out);
    return out;
}
/**
...
function normalizeAllCapsHeadings(text) {
    return text.replace(/^(#{1,6}[ \t]+)(.+)$/gm, (_full, hashes, content) => {
 ...
 */
function flattenHeadingExplosion(text) {
 ...
 */
function normalizeListLabels(text) {
    return text.replace(/^([ \t]*[-*+][ \t]+)\*\*([^*\n]+)\*\*(\s*:)/gm, '$1$2$3');
}
/**
 ...
 */
function stripDefaultBold(text) {
    return text.replace(/\*\*([^*\n]+)\*\*/g, '$1');
}
/**
